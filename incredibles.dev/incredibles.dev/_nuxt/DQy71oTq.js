...
var C = class {
    ...
    advance(t) {
        ...
        } else this.lerp ? (this.value = k(this.value, this.to, this.lerp * 60, t), Math.round(this.value) === Math.round(this.to) && (this.value = this.to, i = !0)) : (this.value = this.to, i = !0);
    ...
    fromTo(t, i, {
        ...
        this.from = this.value = t, this.to = i, this.lerp = e, this.duration = o, this.easing = s, this.currentTime = 0, this.isRunning = !0, n ? .(), this.onUpdate = h
    }
};
...
var X = class {
        ...
        constructor(t, i, {
            ...
            this.wrapper = t, this.content = i, e && (this.debouncedResize = D(this.resize, o), this.wrapper instanceof Window ? window.addEventListener("resize", this.debouncedResize) : (this.wrapperResizeObserver = new ResizeObserver(this.debouncedResize), this.wrapperResizeObserver.observe(this.wrapper)), this.contentResizeObserver = new ResizeObserver(this.debouncedResize), this.contentResizeObserver.observe(this.content)), this.resize()
        }
        destroy() {
...
var P = class {
    ...
    onTouchMove = t => {
        const {
            clientX: i,
            clientY: e
        } = t.targetTouches ? t.targetTouches[0] : t, o = -(i - this.touchStart.x) * this.options.touchMultiplier, s = -(e - this.touchStart.y) * this.options.touchMultiplier;
        this.touchStart.x = i, this.touchStart.y = e, this.lastDelta = {
            x: o,
            y: s
        }, this.emitter.emit("scroll", {
    ...
    onWheel = t => {
        ...
        const s = L(o, this.window.width),
            n = L(o, this.window.height);
        i *= s, e *= n, i *= this.options.wheelMultiplier, e *= this.options.wheelMultiplier, this.emitter.emit("scroll", {
            deltaX: i,
            deltaY: e,
            event: t
        })
...
const N = t => Math.min(1, 1.001 - 2 ** (-10 * t));
var A = class {
    _isScrolling = !1;
    _isStopped = !1;
    _isLocked = !1;
    ...
    constructor({
        ...
        gestureOrientation: r = p === "horizontal" ? "both" : "vertical",
        touchMultiplier: u = 1,
        wheelMultiplier: l = 1,
        autoResize: m = !0,
        prevent: v,
        ...
        window.lenisVersion = z, window.lenis || (window.lenis = {}), window.lenis.version = z, p === "horizontal" && (window.lenis.horizontal = !0), s === !0 && (window.lenis.touch = !0), (!t || t === document.documentElement) && (t = window), typeof c == "number" && typeof a != "function" ? a = N : typeof a == "function" && typeof c != "number" && (c = 1), this.options = {
            wrapper: t,
            content: i,
            eventsTarget: e,
            smoothWheel: o,
            ...
            capture: !0
        }), (this.options.anchors || this.options.stopInertiaOnNavigate) && this.options.wrapper.addEventListener("click", this.onClick), this.options.wrapper.addEventListener("pointerdown", this.onPointerDown), this.virtualScroll = new P(e, {
            touchMultiplier: u,
            wheelMultiplier: l
    ...
    onClick = t => {
        const i = t.composedPath().filter(o => o instanceof HTMLAnchorElement && o.href).map(o => new URL(o.href)),
    ...
    onVirtualScroll = t => {
        ...
        const c = this.options.gestureOrientation === "vertical" && e === 0 || this.options.gestureOrientation === "horizontal" && i === 0;
        if (h || c) return;
        let a = o.composedPath();
        a = a.slice(0, a.indexOf(this.rootElement));
        const d = this.options.prevent,
            f = Math.abs(i) >= Math.abs(e) ? "horizontal" : "vertical";
        ...
        let p = e;
        this.options.gestureOrientation === "both" ? p = Math.abs(e) > Math.abs(i) ? e : i : this.options.gestureOrientation === "horizontal" && (p = i), (!this.options.overscroll || this.options.infinite || this.options.wrapper !== window && this.limit > 0 && (this.animatedScroll > 0 && this.animatedScroll < this.limit || this.animatedScroll === 0 && e > 0 || this.animatedScroll === this.limit && e < 0)) && (o.lenisStopPropagation = !0), o.cancelable && o.preventDefault();
        const r = s && this.options.syncTouch,
            u = s && o.type === "touchend";
        u && (p = Math.sign(p) * Math.abs(this.velocity) ** this.options.touchInertiaExponent), this.scrollTo(this.targetScroll + p, {
            programmatic: !1,
            ...r ? {
    ...
    raf = t => {
        ...
        this.time = t, this.animate.advance(i * .001), this.options.autoRaf && (this._rafId = requestAnimationFrame(this.raf))
    };
    scrollTo(t, {
        offset: i = 0,
        immediate: e = !1,
        ...
        if (typeof r == "string" && ["top", "left", "start", "#"].includes(r)) r = 0;
            ...
            if (typeof r == "string" ? (l = document.querySelector(r), l || (r === "#top" ? r = 0 : console.warn("Lenis: Target not found", r))) : r instanceof HTMLElement && r ? .nodeType && (l = r), l) {
                if (this.options.wrapper !== window) {
                    ...
                    u -= this.isHorizontal ? y.left : y.top
                }
                const m = l.getBoundingClientRect(),
                    v = getComputedStyle(l),
                    S = this.isHorizontal ? Number.parseFloat(v.scrollMarginLeft) : Number.parseFloat(v.scrollMarginTop),
                    w = getComputedStyle(this.rootElement),
                    g = this.isHorizontal ? Number.parseFloat(w.scrollPaddingLeft) : Number.parseFloat(w.scrollPaddingTop);
                r = (this.isHorizontal ? m.left : m.top) + this.animatedScroll - (Number.isNaN(S) ? 0 : S) - (Number.isNaN(g) ? 0 : g)
            }
        }
        if (typeof r == "number") {
            if (r += u, this.options.infinite) {
                if (s) {
                    this.targetScroll = this.animatedScroll = this.scroll;
                    const l = r - this.animatedScroll;
                    l > this.limit / 2 ? r -= this.limit : l < -this.limit / 2 && (r += this.limit)
                }
            } else r = O(0, r, this.limit);
            if (r === this.targetScroll) {
                a ? .(this), d ? .(this);
                return
            }
            if (this.userData = p ? ? {}, e) {
            ...
            s || (this.targetScroll = r), typeof h == "number" && typeof c != "function" ? c = N : typeof c == "function" && typeof h != "number" && (h = 1), this.animate.fromTo(this.animatedScroll, r, {
                duration: h,
                easing: c,
                lerp: n,
                onStart: () => {
    ...
    hasNestedScroll(t, {
        ...
        t._lenis || (t._lenis = {});
        const s = t._lenis;
        let n, h, c, a, d, f, p, r, u, l;
        if (o - (s.time ? ? 0) > 2e3) {
            s.time = Date.now();
            const b = window.getComputedStyle(t);
            if (s.computedStyle = b, n = ["auto", "overlay", "scroll"].includes(b.overflowX), h = ["auto", "overlay", "scroll"].includes(b.overflowY), d = ["auto"].includes(b.overscrollBehaviorX), f = ["auto"].includes(b.overscrollBehaviorY), s.hasOverflowX = n, s.hasOverflowY = h, !(n || h)) return !1;
            p = t.scrollWidth, r = t.scrollHeight, u = t.clientWidth, l = t.clientHeight, c = p > u, a = r > l, s.isScrollableX = c, s.isScrollableY = a, s.scrollWidth = p, s.scrollHeight = r, s.clientWidth = u, s.clientHeight = l, s.hasOverscrollBehaviorX = d, s.hasOverscrollBehaviorY = f
        } else c = s.isScrollableX, a = s.isScrollableY, n = s.hasOverflowX, h = s.hasOverflowY, p = s.scrollWidth, r = s.scrollHeight, u = s.clientWidth, l = s.clientHeight, d = s.hasOverscrollBehaviorX, f = s.hasOverscrollBehaviorY;
        if (!(n && c || h && a)) return !1;
        const m = Math.abs(i) >= Math.abs(e) ? "horizontal" : "vertical";
        let v, S, w, g, y, T;
        if (m === "horizontal") v = Math.round(t.scrollLeft), S = p - u, w = i, g = n, y = c, T = d;
        else if (m === "vertical") v = Math.round(t.scrollTop), S = r - l, w = e, g = h, y = a, T = f;
        else return !1;
        return !T && (v >= S || v <= 0) ? !0 : (w > 0 ? v < S : v > 0) && g && y
    }
    get rootElement() {
